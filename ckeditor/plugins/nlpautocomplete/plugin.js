CKEDITOR.plugins.add( 'nlpautocomplete', {
init: function(editor) {
        var pluginDirectory = this.path;
        editor.addContentsCss( pluginDirectory + 'styles/plugin.css' );
...
function getSuggestions(words, amount, coalback) {
	...
	timeout = setTimeout(function() {
		$.ajax({
			url: 
				"https://nlp.fi.muni.cz/projekty/predictive/predict.py?input=" + 
				stringForServer,
...
function lastWord(t) {
  //console.log(t)
	//return t.split(/[\n ,-]+/).pop();
  var text = t.html().replace(/<\/?p>/g,'').replace(/<br>/g,' ');
  var text_split = text.split(/[\n ,-]+/);
  return text_split[text_split.length-2]
}
...
function insertCompletion(editorId, shadowId) {
	suggestion = lastWord($('#' + shadowId));
	currentWord = lastWord($('#' + editorId));
...
function restorePosition(pos, el) {
  ...
  if (el.firstChild.nodeName == 'P') {
  	var startNode = el.childNodes[el.childNodes.length-1].firstChild;
	  var endNode = el.childNodes[el.childNodes.length-1].firstChild;
  } else {
	...
	//range.setStart(startNode, pos-1);
	//range.setEnd(endNode, pos-1);
...
function showAlternatives(editorId) {
	...
	$.each( $("#alternatives span"), function(index, span) {
		$(span).css("margin-right", "10px");
	});
	//$("#alternatives .current").attr("style", function(i,s) { return s + "color: #333 !important"});
...
function replaceCompletion(completion, editorId, shadowId) {
 ...
 // var matches = editorText.match(/( |<p>)/g);
  //matches[matches.length - 1];
  n = editorText.regexLastIndexOf(/( |<p>)/)
  if (editorText[n] == '<') {
    n = n+2;
  }
...
$( document ).ready(function() {
	...
	var alternativesSpanProps = {
		"margin-right": "6px",
		"color": "6px"
	};

	var alternativesCurrent = {
		"color": "#333 !important"
	};

	var alternativesProps = {
		"display": "none", 
		"position": "absolute", 
		"min-width": "10px", 
		"font-size": "80%", 
	...
	//var editorPropsArr = ["width", "height", "font-size", "font-family"]

	document.getElementById(editor.name).addEventListener("keydown", onKeyDown);
	document.getElementById(editor.name).addEventListener("keyup", onKeyUp);
	document.getElementById(editor.name).addEventListener("keypress", onKeyPress);

	function onKeyPress(e) {
		keyCount++;
		//console.log(keyCount);
	};
	$("#"+editorId).wrap("<div class='wrapper' id='wrapper1'></div>");
	//var editorProps = $("#editor1").css(editorPropsArr);
	$("#"+editorId).before('<div class="ckeditor" id=' +shadowId +'></div>')
	//console.log(editorProps);
	$("#"+shadowId).css(shadowProps);
	$("#wrapper1").after('<div id="alternatives"></div>');
	$("#"+editorId).addClass("nlpautocomplete");
	$("#"+shadowId).addClass("nlpautocomplete");
	$("#alternatives").css(alternativesProps);
	$("#wrapper1").css(wrapper1Props);
	$.each( $("#alternatives span"), function(index, span) {
	...
	function onKeyDown(e) {
		if (e.keyCode == 9 && tabPress == true) {
			...
			// beforehand
			previous = suggestIndex>0 ? (suggestIndex - 1) : (suggestions.length - 1);
